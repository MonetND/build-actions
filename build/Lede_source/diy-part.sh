...
#uci set network.lan.gateway='192.168.2.3'                                  # IPv4 网关
...
uci set system.@system[0].hostname='OpenWrt-B70'                            # 修改主机名称为OpenWrt-123
sed -i 's/\/bin\/login/\/bin\/login -f root/' /etc/config/ttyd              # 设置ttyd免帐号登录，如若开启，进入OPENWRT后可能要重启一次才生效（去掉uci前面的#生效）
EOF

sed -i "s/OpenWrt /莫奈和 compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ           # 增加个性名字 ${Author} 默认为你的github帐号
...
rm -rf openwrt-ramips-mt7621-hiwifi_hc5962-initramfs-kernel.bin
rm -rf openwrt-ramips-mt7621-hiwifi_hc5962.manifest
rm -rf openwrt-ramips-mt7621-hiwifi_hc5962-squashfs-sysupgrade.bin
rm -rf sha256sums
